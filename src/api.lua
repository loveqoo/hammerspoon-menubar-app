...
function M.init()
    ...
    M.apis = {
        ...
        {
            name = "weather",
            url = string.format("%s?q=%s,%s&appid=%s&units=%s",
                config.api.base_url,
                config.weather.city,
...
function M.makeRequest(apiConfig, callback)
    ...
    local method = apiConfig.method or "GET"
    local headers = apiConfig.headers or {}
    local timeout = apiConfig.timeout or 10
...
function M.fetchAllApis(callback)
    ...
    for i, apiConfig in ipairs(M.apis) do
...
-- 새 API 설정 추가
function M.addApi(name, url, config)
    local apiConfig = {
        name = name,
        url = url,
        method = config.method or "GET",
        headers = config.headers or {},
        timeout = config.timeout_sec or 10,
